...
app.get("/pokemons/detail/", async (req: any, res: any) => {
  ...
  try {
    ...
    const pokemonDescriptionResponse = await fetchPokemonInfo(
      "characteristic",
      pokemonId
    );
    const pokemonGenderResponse = await fetchPokemonInfo("gender", pokemonId);
    const pokemonHabitatResponse = await fetchPokemonInfo(
      "pokemon-habitat",
      pokemonId
    );
...
app.get("/pokemons/list/", async (req: any, res: any) => {
  ...
  try {
    ...
    const listOfPokemonsToFetch = getListOfPokemonsToFetch(
      name,
      startIndex,
      endIndex
    );
    ...
    const previousAndNextPage = getPreviousAndNextPage(
      pokemonsReferenceList,
      startIndex,
      endIndex,
      page,
      limit
    );
    const listPokemonsResult = {
      ...previousAndNextPage,
      pokemonsList: pokemonsListFiltered,
    };
...
const fetchPokemonInfo = async (
  ...
  let pokemonResponse;
  try {
    pokemonResponse = await axios.get(
      `https://pokeapi.co/api/v2/${pokemonDetail}/${pokemonId}/`
    );
  } catch {
...
const getPokemonDescriptionValue = (pokemonResponse: any): string => {
  ...
  const completePokemonDescription = pokemonResponse.data.descriptions.find(
    (description: any) => {
...
const getAllPokemonsReference = async (): Promise<any> => {
  const allPokemonsReferenceResponse = await axios.get(
    `https://pokeapi.co/api/v2/pokemon/`,
    { params: { limit: 1200, page: 1 } }
  );
...
const getListOfPokemonsToFetch = (
  name: string,
  startIndex: number,
  endIndex: number
): any[] => {
...
const fetchPokemons = async (pokemonsNamesList: string[]): Promise<any[]> => {
  const myArray = await Promise.all(
    pokemonsNamesList.map(async (pokemonName) => {
      const actualPokemon = await axios.get(
...
const filterPokemonsProperties = (
  pokemonsList: any[]
): PokemonQuickViewResponse[] => {
